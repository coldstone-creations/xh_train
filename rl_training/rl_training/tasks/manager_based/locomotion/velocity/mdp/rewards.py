...
def stand_still_vel_penalty(
    ...
    """Penalize body xy velocity when command is near zero (robot should stand still).

    This directly punishes backward/sideways drift during standing, which the
    exponential tracking reward is too lenient to prevent.
    """
    asset: RigidObject = env.scene[asset_cfg.name]
    # Use gravity-aligned body frame velocity for consistent measurement
    vel_xy = torch.norm(asset.data.root_lin_vel_b[:, :2], dim=1)
    # Only active when command is below threshold (standing)
    reward = vel_xy * (torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1) < command_threshold)
    return reward
