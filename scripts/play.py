...
# parser.add_argument("--camera_follow", action="store_true", default=False, help="Enable camera follow mode.")
# parser.add_argument("--flat_terrain", action="store_true", default=False, help="Use flat plane terrain instead of rough terrain.")
# parser.add_argument("--lin_vel_x", type=float, default=None, help="Fixed forward velocity (m/s). Range: [-1.5, 1.5]")
# parser.add_argument("--lin_vel_y", type=float, default=None, help="Fixed lateral velocity (m/s). Range: [-0.8, 0.8]")
# parser.add_argument("--ang_vel_z", type=float, default=None, help="Fixed yaw rate (rad/s). Range: [-0.8, 0.8]")
...
# from scripts.rl_utils import camera_follow
...
#     # override terrain to flat plane if requested
#     if args_cli.flat_terrain:
#         env_cfg.scene.terrain.terrain_type = "plane"
#         env_cfg.scene.terrain.terrain_generator = None
#         # remove height scan observation (no height variation on flat terrain)
...
#     # DEBUG: let robot settle, then print contact forces by body
#     print("[DEBUG] Letting robot settle for 1 second...")
...
#     # Get contact force magnitude for each body (latest frame)
#     net_forces = contact_sensor.data.net_forces_w[:, :, :].norm(dim=-1)  # (num_envs, num_bodies)

#     print(f"[DEBUG] After settling - Base height: {base_z:.4f} m")
#     print(f"[DEBUG] Body contact forces (only bodies with force > 0.1N):")
#     for i, name in enumerate(contact_sensor.body_names):
#         force = net_forces[0, i].item()
#         if force > 0.1:
#             z = robot.data.body_pos_w[0, i, 2].item()
#             print(f"  {'[FOOT]' if 'foot' in name.lower() else '       '} {name}: force={force:.2f}N, z={z:.4f}, rel_base={z-base_z:+.4f}")

#     # Override velocity commands with fixed values if specified
#     fixed_vel_cmd = None
#     if args_cli.lin_vel_x is not None or args_cli.lin_vel_y is not None or args_cli.ang_vel_z is not None:
#         vx = args_cli.lin_vel_x if args_cli.lin_vel_x is not None else 0.0
#         vy = args_cli.lin_vel_y if args_cli.lin_vel_y is not None else 0.0
#         wz = args_cli.ang_vel_z if args_cli.ang_vel_z is not None else 0.0
#         fixed_vel_cmd = torch.tensor([[vx, vy, wz]], dtype=torch.float32, device=env.unwrapped.device)
#         print(f"[INFO] Using fixed velocity command: lin_vel_x={vx}, lin_vel_y={vy}, ang_vel_z={wz}")
...
#             # Override velocity command AFTER step (step calls compute() which would overwrite)
#             if fixed_vel_cmd is not None and not args_cli.keyboard:
#                 env.unwrapped.command_manager.get_command("base_velocity")[:] = fixed_vel_cmd
...
#         if args_cli.keyboard or args_cli.camera_follow:
#             camera_follow(env)
...
# local imports — ensure local rl_training takes precedence over pip-installed version
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    while simulation_app.is_running():
        ...
        VIS_ENABLED = False  # debug visualization disabled
        VIS_REF_ENABLE = False
        if (
            VIS_ENABLED
            and draw_interface is not None
